...
class QueueManager extends DataBus
{
	...
	public function runCommand($cmd)
	{
		...
		$command = "cd $path && nohup ./yiic $cmd > /dev/null 2> /dev/null &";  
			...
			case "XlsRecordEmpty":
           	...
           	case "ProcessStack": 
           		...
           		foreach ( $qm_files as $file_data ) 
           		{
           			...
           			if ( !($file_data['is_complete'] > 0 ) && time() - $file_data['ts_upd'] > 60 && ( $file_data['gb_call'] > 0 || $file_data['mk_call'] > 0 ) )
           			{
         				$this->_storage_save($x_id, array('$set'=>array( 'gb_call' => 0, 'mk_call' => 0 )));
           				#$this->triggerMKCompleteAllRequests(array('x' => str_replace( 'x','', $x_id )));
